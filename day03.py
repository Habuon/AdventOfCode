...
class Solution(AocMeta):
	def process_input(self, inp: str) -> dict:
		bateries = [line.strip() for line in inp.split("\n")]
		return {"bateries": bateries}
		
	def find_biggest_number(self, bateries, number_length):
		start = 0
		result = []

		for position in range(number_length):
		    last_index = len(bateries) - (number_length - position)
		    best_idx, best_digit = start, '0'

		    for index in range(start, last_index + 1):
		        if bateries[index] > best_digit:
		            best_digit, best_idx = bateries[index], index		            
		            if best_digit == '9':
		                break
	...
	def first_star(self, inp):
		result = 0
		for bateries in inp["bateries"]:
			biggest_number = self.find_biggest_number(bateries, 2)
	...
	def second_star(self, inp):
		result = 0
		for bateries in inp["bateries"]:
			biggest_number = self.find_biggest_number(bateries, 12)
			result += biggest_number
		return result
